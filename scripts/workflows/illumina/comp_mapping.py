...
def dump_reference(name):
    ...
    refs = [s.references[sp] for sp in species]
    ...
    with open(join(s.work, s_trgt['name'], 'reference.fasta'), 'w') as handle:
        SeqIO.write(out, handle, 'fasta')


def map_reads(name):
    s_trgt = get_sample(name)
    out_f = join(s.work, s_trgt['name'], 'mapped_reads')
    ref = join(s.work, s_trgt['name'], 'reference.fasta')
